...
class _UserModel(nn.Module):
    ...
    def forward(self, uids, u_item_pad):
        # item aggregation
        q_a = self.item_emb(u_item_pad[:, :, 0])  # B x maxi_len x emb_dim 物品作Embedding
        mask_u = torch.where(u_item_pad[:, :, 0] > 0, torch.tensor([1.], device=self.device),
                             torch.tensor([0.], device=self.device))  # B x maxi_len  判断是否都是确定有项目的，有物品为1,无物品为0
        u_item_er = self.rate_emb(u_item_pad[:, :, 1])  # B x maxi_len x emb_dim 评分作Embedding
...
class DRGR(nn.Module):
    ...
    def forward(self, uids, iids, u_itemdiv_pad, u_avgs, u_user_pad, u_user_item_pad, i_user_pad, i_avgs):
        ...
        h_i = self.user_model(uids, u_itemdiv_pad)  # B x emb_dim
        ...
        r_ij = 0.5 *(r_ij_1 + r_ij_2) + 0.8*0.5*(u_avgs.unsqueeze(1) + i_avgs.unsqueeze(1))
